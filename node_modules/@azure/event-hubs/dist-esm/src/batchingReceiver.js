...
import { translate, Constants, MessagingError } from "@azure/amqp-common";
    ...
    receive(maxMessageCount, maxWaitTimeInSeconds) {
        ...
        return new Promise((resolve, reject) => {
            ...
            actionAfterWaitTimeout = () => {
                timeOver = true;
                log.batching("[%s] Batching Receiver '%s', %d messages received when max wait time in seconds %d is over.", this._context.connectionId, this.name, eventDatas.length, maxWaitTimeInSeconds);
                return finalAction(timeOver);
            };
            // Action to be performed on the "message" event.
            onReceiveMessage = (context) => {
                const data = EventData.fromAmqpMessage(context.message);
                ...
                if (eventDatas.length === maxMessageCount) {
                    log.batching("[%s] Batching Receiver '%s', %d messages received within %d seconds.", this._context.connectionId, this.name, eventDatas.length, maxWaitTimeInSeconds);
            ...
            onReceiveClose = (context) => __awaiter(this, void 0, void 0, function* () {
                const receiverError = context.receiver && context.receiver.error;
                if (receiverError) {
                    log.error("[%s] 'receiver_close' event occurred. The associated error is: %O", this._context.connectionId, receiverError);
                }
            });
            onSessionClose = (context) => __awaiter(this, void 0, void 0, function* () {
