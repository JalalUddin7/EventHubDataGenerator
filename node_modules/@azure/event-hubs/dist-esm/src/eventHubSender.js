...
import { messageProperties, SenderEvents, message } from "rhea-promise";
import { defaultLock, retry, translate, ErrorNameConditionMapper, RetryOperationType, Constants, randomNumberFromInterval } from "@azure/amqp-common";
...
export class EventHubSender extends LinkEntity {
    ...
    constructor(context, partitionId, name) {
        ...
        this._onAmqpClose = (context) => __awaiter(this, void 0, void 0, function* () {
            ...
            const senderError = context.sender && context.sender.error;
            if (senderError) {
                log.error("[%s] 'sender_close' event occurred for sender '%s' with address '%s'. " + "The associated error is: %O", this._context.connectionId, this.name, this.address, senderError);
            }
        ...
        this._onSessionClose = (context) => __awaiter(this, void 0, void 0, function* () {
            ...
            const sessionError = context.session && context.session.error;
            if (sessionError) {
                log.error("[%s] 'session_close' event occurred for sender '%s' with address '%s'. " + "The associated error is: %O", this._context.connectionId, this.name, this.address, sessionError);
            }
    ...
    detached(senderError) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                log.error("[%s] An error occurred while processing detached() of Sender '%s' with address " + "'%s': %O", this._context.connectionId, this.name, this.address, err);
            }
        });
    }
    /**
    ...
    send(data) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                if (!this.isOpen()) {
                    log.sender("Acquiring lock %s for initializing the session, sender and " + "possibly the connection.", this.senderLock);
                    yield defaultLock.acquire(this.senderLock, () => {
                        return this._init();
                    });
                }
    ...
    _trySend(message, tag, format) {
        const sendEventPromise = () => new Promise((resolve, reject) => {
            ...
            if (this._sender.sendable()) {
                log.sender("[%s] Sender '%s', sending message with id '%s'.", this._context.connectionId, this.name, (Buffer.isBuffer(message) ? tag : message.message_id) || tag || "<not specified>");
                let onRejected;
                let onReleased;
                let onModified;
                let onAccepted;
        ...
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                if (!this.isOpen() && !this.isConnecting) {
                    ...
                    log.error("[%s] The sender '%s' with address '%s' is open -> %s and is connecting " + "-> %s. Hence not reconnecting.", this._context.connectionId, this.name, this.address, this.isOpen(), this.isConnecting);
                }
            }
            catch (err) {
                this.isConnecting = false;
